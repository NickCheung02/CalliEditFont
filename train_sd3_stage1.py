...
def main(args):
    ...
    # ===== 新增：加载 PPOCRv3 作为静态的风格特征提取器 =====
    logger.info("Loading PPOCRv3 as Style Extractor...")
    rec_config = edict(
        in_channels=3,
    ...
    ocr_model.requires_grad_(False) # 严格冻结，不参与训练
    ocr_model.eval()
    # ========================================================

    # wrapper multi models (新增：开启风格控制开关 use_style_cond=True)
    # 假设 OCR 序列输出长 40，通道 64，展平为 2560
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(wrapper_model):
                ...
                # ===== 新增：提取风格特征 =====
                # batch['style_image'] 形状为 [B, max_num_texts, 3, 48, 320]
                # 我们取第一行文字的图片 [:, 0, ...] 作为整张海报的"全局书法风格"。
                # 这样刚好匹配 WrapperModel 中广播到所有 token 的逻辑。
                style_images = batch['style_image'][:, 0, :, :, :].to(
                    memory_format=torch.contiguous_format, dtype=weight_dtype, device=accelerator.device
                )
                with torch.no_grad():
                    # 调用 PPOCR 的 encode，只取 Neck 特征，形状为 [B, 40, 64]
                    style_features = ocr_model.encode(style_images)
                # ===============================
                ...
                wrapper_rel = wrapper_model(
                    ...
                    controlnet_cond=cond_latents_text,
                    text_embeds=text_embeds,
                    style_features=style_features, # 新增：送入提取好的 OCR 特征

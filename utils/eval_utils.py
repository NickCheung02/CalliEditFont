...
# 修改函数签名，增加 ocr_model
def process(batch, args, pipeline, ocr_model=None):
    with torch.no_grad():
        prompt = batch['caption']        
        bg_im = batch['bg_im'].float().cuda() # [-1,1], BCHW
        controlnet_im = batch['controlnet_im'].float().cuda()
        ...
        # ===== 新增：提取风格特征 =====
        style_features = None
        if ocr_model is not None and 'style_image' in batch:
            # 同样取第一行字的图片作为全局风格
            style_images = batch['style_image'][:, 0, :, :, :].float().cuda()
        ...
        image = pipeline(
            ...
            controlnet_conditioning_scale=1.0,
            guidance_scale=args.cfg_scale,
            style_features=style_features, # ===== 新增：传入风格特征 =====
            enable_style=True              # ===== 新增：验证时默认开启风格 =====
        ).images[0]
...
def log_validation_with_pipeline(logger, pipeline, dataloader, args, accelerator, step, ocr_model=None):
    ...
    with torch.no_grad():
        for i, batch in tqdm(enumerate(dataloader)):
            with torch.autocast("cuda"):
                # ===== 修改调用 process 的地方 =====
                result = process(batch, args, pipeline, ocr_model=ocr_model) 
